...
/* Coefficient matrix for interpolating intensity and gradient */
{
	...
	mat v = {{1 + 6 * lamda_, -3 - 6 * lamda_, 3, -1},
			{4 - 12 * lamda_, 18 * lamda_, -6, 3},
			{1 + 6 * lamda_, 3 - 18 * lamda_, 3, -3},
			{0,	6 * lamda_, 0, 1}};
	bsv_ = v / (1 + 6 * lamda_);

	// Coefficient matrix for interpolating gradient
	mat g = {{-1 - 2 * lamda_, 2, -1},
			{6 * lamda_, -4, 3},
			{1 - 6 * lamda_, 2, -3},
			{2 * lamda_, 0,	1}};
	bsg_ = 3 * g / (1 + 6 * lamda_);
...
double BicubicMOMSInterpolator::Value(const double& x, const double& y) const
{
	...
	vec deltax = { 1, ex, ex * ex, ex * ex * ex };
	vec deltay = { 1, ey, ey * ey, ey * ey * ey };

	const mat coeff = lut_.submat(r - 1, c - 1, r + 2, c + 2);
	const mat aa = coeff * bsv_;
	const mat bb = bsv_.t() * aa;
	const vec cc = bb * deltax;
	const double value = dot(deltay, cc);
...
double BicubicMOMSInterpolator::GradientX(const double& x, const double& y) const
{
	...
	vec deltax = { 1, ex, ex * ex };
	vec deltay = { 1, ey, ey * ey, ey * ey * ey };

	const mat coeff = lut_.submat(r - 1, c - 1, r + 2, c + 2);
	const mat aa = coeff * bsg_;
	const mat bb = bsv_.t() * aa;
	const vec cc = bb * deltax;
	const double value = dot(deltay, cc);
...
double BicubicMOMSInterpolator::GradientY(const double& x, const double& y) const
{
	...
	vec deltax = { 1, ex, ex * ex, ex * ex * ex };
	vec deltay = { 1, ey, ey * ey };

	const mat coeff = lut_.submat(r - 1, c - 1, r + 2, c + 2);
	const mat aa = coeff * bsv_;
	const mat bb = bsg_.t() * aa;
	const vec cc = bb * deltax;
	const double value = dot(deltay, cc);
